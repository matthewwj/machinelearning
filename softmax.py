...
def softmax(X):
    ...
    Args: X: numpy array shape (n, d) each row is a data point Returns: res: numpy array shape (n, d)  where each row
    is the softmax transformation of the corresponding row in X i.e. res[i, :] = softmax(X[i, :])
    """
...
class SoftmaxClassifier:
    ...
    def cost_grad(self, X, y, W):
        ...
        stabilized_scores = scores - max_scores  # to avoid numerical issues
        ...
        cost = -np.mean(np.sum(Yk * np.log(softmax_probabilities + 1e-10), axis=1))

        n = X.shape[0]
        grad = -X.T @ (Yk - softmax_probabilities) / n
